...
class Day7 {
    ...
    @Test
    fun day7_find_all_bags_including_shiny_gold_ones() {
        val rulesAsMap = getRulesAsMap(puzzleInput)

        val results = mutableSetOf<String>()
        var currentKeysToLookFor = setOf("shiny gold")
        while (currentKeysToLookFor.isNotEmpty()) {
            currentKeysToLookFor = rulesAsMap.filter {
                it.value.keys.intersect(currentKeysToLookFor).isNotEmpty()
            }.keys
            results.addAll(currentKeysToLookFor)
        }
        println(results.size)
    }

    @Test
    fun day7_find_all_bags_required_for_shiny_gold_one() {
        val testInput = """
        ...
        val subElements = getSubElements(rulesAsMap, start)
        println(subElements.map { it.value }.sum())
    }

    private fun getSubElements(rulesAsMap: Map<String, Map<String, Int>>, subKey: String): Map<String, Int> {
        val subMap = rulesAsMap[subKey] ?: return mapOf()
        val result = mutableMapOf<String, Int>()
        for (entry in subMap) {
            val from = getSubElements(rulesAsMap, entry.key)
            for (subElementEntry in from) {
                result[subElementEntry.key] = (result[subElementEntry.key] ?: 0) + entry.value * subElementEntry.value
            }
    ...
    private fun getRulesAsMap(input: String): MutableMap<String, Map<String, Int>> {
        val rules = input.lines()
        val rulesAsMap = mutableMapOf<String, Map<String, Int>>()
        for (rule in rules) {
            val rulesByBag = rule.split(" bag(s)?( contain)?(,)?(\\.)?".toRegex())
                .map { it.trim() }
                .filter { it.isNotEmpty() }
            val key = rulesByBag.first()
            val bagRules = rulesByBag.slice(1 until rulesByBag.size)
            val pairs = bagRules.map { bagRule ->
                val singleRule = bagRule.split("\\s".toRegex(), 2)
                val first = singleRule.first()
                val quantity = first.toIntOrNull() ?: 0
                val color = singleRule.last()
                Pair(color, quantity)
            }.filter { it.first != "other" && it.second > 0 }
                .toMap()
            rulesAsMap[key] = pairs
        }
        return rulesAsMap
    }
