...
class Day6 {
    ...
    @Test
    fun part1_anyone() {
        val groups = puzzleInput.split(Regex("\\s+\\n"))
        var questionsAnsweredWithYes = 0
        for (group in groups) {
            questionsAnsweredWithYes += group.split(Regex("\\n"))
                .joinToString(separator = "") { it }
                .toSet().size
        }
        println(questionsAnsweredWithYes)
    }
    ...
    fun part2_everyone() {
        ...
        var questionsAnsweredWithYes = 0
        for (group in groups) {
            val answersPerPerson = group.split(Regex("\\n"))
            var firstPersonAnswers = answersPerPerson.joinToString(separator = "") { it }.toSet()
            for (personAnswer in answersPerPerson) {
                val personsAnswers = personAnswer.toSet()
                firstPersonAnswers = personsAnswers.intersect(firstPersonAnswers)
            }
            questionsAnsweredWithYes += firstPersonAnswers.size
        }
        println(questionsAnsweredWithYes)
    }
