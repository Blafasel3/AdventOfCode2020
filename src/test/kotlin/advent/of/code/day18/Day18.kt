...
class Day18 {
    ...
    @Test
    fun `part 1 with no precedence`() {
        println(calculateSumWithPrecedenceFunction(puzzleInput, this::reduceSingleBlockNoPrecedence))
    }

    @Test
    fun `part 2 with reversed precedence`() {
        println(calculateSumWithPrecedenceFunction(puzzleInput, this::reduceSingleBlockWithReversedPrecedence))
    }

    private fun calculateSumWithPrecedenceFunction(
        input: String,
        reducerFunction: (String) -> MutableList<String>
    ): BigInteger {
        val lines = input.lines()
        return lines.map {
            var line = it
            while (line.contains('(')) {
                val lastOpenBraceIndex = line.lastIndexOf('(')
                val accordingClosingBraceIndex = line.indexOf(')', startIndex = lastOpenBraceIndex)
                val operation = line.substring(lastOpenBraceIndex + 1, accordingClosingBraceIndex)
                val split = reducerFunction(operation)
                line = line.replaceRange(lastOpenBraceIndex..accordingClosingBraceIndex, split[0])
            }
            val split = reducerFunction(line)
            split[0].toBigInteger()
        }.reduce(BigInteger::add)
    }

    private fun reduceSingleBlockNoPrecedence(operation: String): MutableList<String> {
        val split = operation.split(" ").toMutableList()
        val validOperations = listOf("*", "+")
        while (split.contains("*") || split.contains("+")) {
            val indexOf = split.indexOfFirst { validOperations.contains(it) }
            val previousNumber = split[indexOf - 1].toBigInteger()
            val nextNumber = split[indexOf + 1].toBigInteger()
            if (split[indexOf] == "*") {
                split[indexOf] = previousNumber.times(nextNumber).toString()
            } else {
                split[indexOf] = previousNumber.plus(nextNumber).toString()
            }
            split.removeAt(indexOf + 1)
            split.removeAt(indexOf - 1)
        }
        return split
    }

    private fun reduceSingleBlockWithReversedPrecedence(operation: String): MutableList<String> {
        val split = operation.split(" ").toMutableList()
        while (split.contains("+")) {
            val indexOf = split.indexOf("+")
            val previousNumber = split[indexOf - 1].toBigInteger()
            val nextNumber = split[indexOf + 1].toBigInteger()
            split[indexOf] = previousNumber.plus(nextNumber).toString()
            split.removeAt(indexOf + 1)
