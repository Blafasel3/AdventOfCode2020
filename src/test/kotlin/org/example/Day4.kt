...
class Day4 {
    private val eyeColors = arrayOf(
        "amb", "blu", "brn", "gry", "grn", "hzl", "oth"
    )

    private val validators = mapOf(
        "byr" to { s: String -> Integer.parseInt(s) in 1920..2002 },
        "iyr" to { s: String -> Integer.parseInt(s) in 2010..2020 },
        "eyr" to { s: String -> Integer.parseInt(s) in 2020..2030 },
        "hgt" to { s: String -> validateHeight(s) },
        "hcl" to { s: String -> Regex("#[a-f\\d]{6}").matches(s) },
        "ecl" to { s: String -> eyeColors.contains(s) },
    ...
    private fun validateHeight(hgt: String): Boolean {
        return when {
            hgt.endsWith("cm") -> {
                val height = hgt.substring(0 until hgt.length - 2)
                val heightInCm = Integer.parseInt(height)
                heightInCm in 150..193
            }
            hgt.endsWith("in") -> {
                val height = hgt.substring(0 until hgt.length - 2)
                val heightInInches = Integer.parseInt(height)
                heightInInches in 59..76
            }
            else -> false
        }
    }
    ...
    fun day4() {
        ...
        for (line in lines) {
            val keysInPassport = line.split(Regex("\\s+"))
                .groupBy(
                    keySelector = { it.substring(0 until 3) },
                    valueTransform = { it.substring(4 until it.length) }
                )
            if (isPassportValid(keysInPassport)) {
                validPassports++
            }
        }
    ...
    private fun isPassportValid(keysInPassport: Map<String, List<String>>): Boolean {
        for (entry in validators) {
            val value = keysInPassport[entry.key]?.get(0) ?: return false // key missing
            val isValid = entry.value(value)
            if (!isValid) {
                return false
            }
        }
