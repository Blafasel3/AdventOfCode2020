...
class Day14 {
    ...
    fun `part 1`() {
        ...
        val memoryAddresses = mutableMapOf<Long, BigInteger>()

        var maskToArray: List<(Char) -> Char> = listOf()
        var currentMask: String
        lines.forEach { line ->
            if (line.startsWith("mask")) {
                currentMask = line.substring(7)
                maskToArray = convertToOperationList(currentMask)
            } else {
                val splitLine = line.split(" = ")
                val numberAsString = splitLine.last()
                val masked = convertToDecimal(numberAsString, maskToArray)
                val memoryAddress = numbersRegex.find(splitLine.first())!!.value.toLong()
                memoryAddresses[memoryAddress] = masked
            }
        }
        println(memoryAddresses.values.reduce(BigInteger::add))
    }
    ...
    fun `part 2`() {
        ...
        lines.forEach { line ->
            if (line.startsWith("mask")) {
                ...
                val splitLine = line.split(" = ")
                val number = splitLine.last().toBigInteger()
                val memoryAddress = numbersRegex
                    .find(splitLine.first())!!
                    .value.toLong()
                applyMaskToMemoryAddress(memoryAddress, currentMask).forEach {
                    memoryAddresses[it] = number
                }
            }
        }
        println(memoryAddresses.values.reduce(BigInteger::add))
    ...
    private fun convertToDecimal(
        ...
        maskToArray: List<(Char) -> Char>
    ): BigInteger {
        val binaryNumber = convertStringToBinary(numberAsString).padStart(36, '0')
        return binaryNumber.mapIndexed { index, char ->
            val function = maskToArray[index]
            function.invoke(char)
        }.joinToString(separator = "") { it.toString() }
            .toBigInteger()
            .getDecimalNumber()
    }

    private fun convertStringToBinary(numberAsString: String) = numberAsString.toLong().toString(2)

    private fun convertToOperationList(mask: String) = mask.split("")
        ...
        .map {
            when (it) {
                ...
                "0" -> zeroMapping
                "1" -> oneMapping
                else -> throw NotImplementedError("Operation $it not supported")
            }
        }

    private fun applyMaskToMemoryAddress(memoryAddress: Long, mask: String): List<Long> {
        var binaryAddresses = listOf("")
        val binaryMemoryAddress = memoryAddress.toString(2)
        mask.forEachIndexed { index, bit ->
            binaryAddresses = if (bit == 'X') {
                binaryAddresses.flatMap {
                    listOf(it + 0, it + 1)
                }
            } else {
                binaryAddresses.map {
                    if (bit == '0')
                        it + binaryMemoryAddress.getOrElse(binaryMemoryAddress.length - mask.length + index) { '0' }
                    else
                        it + bit
                }
            }
        }
        return binaryAddresses.map { it.toLong(2) }
    }
     ...
     */
    private fun BigInteger.getDecimalNumber(): BigInteger {
        var binaryNumber = this
        var decimalNo = BigInteger.ZERO
        var power = 0.0

        while (binaryNumber > BigInteger.ZERO) {
            val r = binaryNumber % BigInteger.TEN
            decimalNo = (decimalNo + r * BigInteger.valueOf(2.0.pow(power).toLong()))
            binaryNumber /= BigInteger.TEN
            power++
        }
        return decimalNo
    }

    private var oneMapping: (i: Char) -> Char = { '1' }
    private var zeroMapping: (i: Char) -> Char = { '0' }
    private var identityMapping: (i: Char) -> Char = { it }
